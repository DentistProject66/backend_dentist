...
const updateAppointment = async (req, res) => {
  try {
    ...
    // Ensure all fields are null if undefined to avoid passing undefined to the query
    const safeAppointmentDate = appointment_date !== undefined ? appointment_date : null;
    const safeAppointmentTime = appointment_time !== undefined ? appointment_time : null;
    const safePatientName = patient_name !== undefined ? patient_name : null;
    const safePatientPhone = patient_phone !== undefined ? patient_phone : null;
    const safeTreatmentType = treatment_type !== undefined ? treatment_type : null;
    ...
    if ((safeAppointmentDate && safeAppointmentDate !== currentAppointment.appointment_date) || 
        (safeAppointmentTime && safeAppointmentTime !== currentAppointment.appointment_time)) {
      
      const existingAppointment = await executeQuery(`
        SELECT id FROM appointments 
        WHERE dentist_id = ? AND appointment_date = ? AND appointment_time = ? 
        AND status IN ('pending', 'confirmed') AND id != ?
      `, [dentistId, safeAppointmentDate || currentAppointment.appointment_date, safeAppointmentTime || currentAppointment.appointment_time, id]);
    ...
    await executeQuery(`
      ...
      SET appointment_date = COALESCE(?, appointment_date), 
          ...
          notes = ?
      WHERE id = ? AND dentist_id = ?
    `, [safeAppointmentDate, safeAppointmentTime, safePatientName, safePatientPhone, safeTreatmentType, safeStatus, safeNotes, id, dentistId]);
